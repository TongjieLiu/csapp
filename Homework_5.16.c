...
void new_inner(vec_ptr u, vec_ptr v, data_t *dest)
{
	...
	data_t *udata, *vdata;
	data_t sum;
	data_t prod0, prod1, prod2, prod3, prod4, prod5;
	...
	limit = length - 5;
	for (i = 0; i < limit; i += 6) {
		prod0 = udata[i] * vdata[i];
		prod1 = udata[i + 1] * vdata[i + 1];
		prod2 = udata[i + 2] * vdata[i + 2];
		prod3 = udata[i + 3] * vdata[i + 3];
		prod4 = udata[i + 4] * vdata[i + 4];
		prod5 = udata[i + 5] * vdata[i + 5];
